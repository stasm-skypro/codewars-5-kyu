...
def get_tree(a: list, flag=True):
    ...
    # Мой способ вычисления положения корневого узла.
    # Не работает на длине исходного списка больше 10.
    # if flag:
    ...
    # Другой способ вычисления полодения корневого узла. Работает на всех тестах.
    height = ceil(log2(len(a) + 1))
    half = [(2**i) // 2 for i in range(height)]
    half[-1] = min(half[-1], len(a) - (2**(height - 1) - 1))
    top = sum(half)
    tree = a[top]
...
def breadth_first(a: list):
    ...
    for i in range(height + 1):
        travers_level(tree, i, res, len(a))
